...
async def upsert_setting(payload: SystemSettignCreate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(selecet(SystemSetting).where(SystemSetting.key == payload.key))
    setting = result.scalar_one_or_none
