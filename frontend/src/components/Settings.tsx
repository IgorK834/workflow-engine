...
export default function Settings() {
  ...
  useEffect(() => {
    getSetting('smtp_profile')
      .then((data) => {
        if (data && data.value) {
          setServer(data.value.server || '');
          setPort(data.value.port?.toString() || '587');
          setLogin(data.value.login || '');
          setPassword(data.value.password || '');
        }
      })
      .catch(() => {
        console.log('Brak zapisanych ustawień SMTP (lub błąd połączenia).');
      });
  }, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Ustawienia zapisano pomyślnie!' });
    } catch (error) {
      console.error(error);
      setStatus({ type: 'error', message: 'Wystąpił błąd podczas zapisywania.' });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="flex-1 bg-muted/30 overflow-auto">
      ...
      <div className="p-8 max-w-3xl">
        <form onSubmit={handleSave} className="bg-white rounded-xl border border-border shadow-sm overflow-hidden">
