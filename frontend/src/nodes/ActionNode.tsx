...
export default function ActionNode({ id, data }: { id: string, data: NodeData }) {
  ...
  return (
    <div className="bg-action-light border border-action/40 rounded-xl p-4 shadow-sm min-w-[240px] relative group">
        <button
      ...
      {data.subtype === 'jira_create_ticket' && (
          <div className="mt-4 pt-3 border-t border-action/20 space-y-3">
              <button 
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 text-[10px] bg-blue-50 text-blue-600 py-1.5 rounded border border-blue-200 hover:bg-blue-100 transition-colors"
              >
              ...
              {projects.length > 0 && (
                  <select 
                      className="w-full text-xs p-2 border rounded bg-white text-slate-700"
              ...
              <input 
                  ...
                  className="w-full text-xs p-2 border rounded bg-blue-50/50 font-medium"
